...
public class TokenReservationPaymentFlowTest extends ReservationIntegrationTestBase {
    ...
    void testTokenReservationPaymentFlow() {
        ...
        // when: 좌석 예약
        MakeReservationResult reservationResult = reserveSeat(user.getId(), concert.getId(), seat.getId());
    ...
    void testSeatReservationAfterExpiration() throws InterruptedException {
        ...
        // 첫 번째 유저 예약
        MakeReservationResult firstReservation = reserveSeat(firstUser.getId(), concert.getId(), seat.getId());

        // 다른 유저 생성 후 예약
        User secondUser = createUser();
        MakeReservationResult secondReservation = reserveSeat(secondUser.getId(), concert.getId(), seat.getId());
    ...
    void testConcurrentSeatRequestsSingleSuccess() throws InterruptedException, ExecutionException {
        ...
        Callable<MakeReservationResult> task = () -> {
            try {
                ...
                return reserveSeat(concurrentUser.getId(), concert.getId(), seat.getId());
            } catch (Exception e) {
                return null;
            }
        };
